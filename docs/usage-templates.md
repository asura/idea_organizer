...
Research Idea Organizer の具体的な使い方を、4つの典型シナリオで紹介する。
...
**ゴール**: 複数のアイデア・仮説から何を採用/保留/却下したかを、判断理由とセットで記録する

### Step 1: 前提となるノードを配置

既存の Question / Hypothesis / Idea / Evidence ノードが配置されている状態を想定：

```
q:単一ベクトルでは駅まち内部回遊が粗すぎる
h:内部配置埋め込みを別立てにすれば改善する
e:現行駅まち埋め込みでは移植性行列を説明できない
i:10×10 token表現を追加する
i:方向・方位・距離エンコードを追加する
i:汎用機能埋め込みと内部配置埋め込みを分離する
```

### Step 2: 判断をDecisionノードで登録

```
Ctrl+K → d:今期は「分離＋後段fusion」を採用、方向エンコード単独投入は保留
```

Decisionノード（🟠橙）には以下の専用フィールドがある：
- **decision_type**: 決定タイプ（adopt / hold / reject / park）
- **rationale**: 判断理由
- **review_trigger**: 再評価条件（どうなったら見直すか）

### Step 3: 判断根拠をエッジで接続

| 接続元 → 接続先 | エッジタイプ | 確信度 |
|---|---|---|
| `e:移植性行列を説明できない` → `d:分離＋後段fusion採用` | SUPPORTS | high |
| `h:内部配置埋め込みを別立て` → `d:分離＋後段fusion採用` | SUPPORTS | medium |
| `d:分離＋後段fusion採用` → `i:汎用と内部配置を分離` | RELATES_TO | high |
| `d:分離＋後段fusion採用` → `i:方向エンコードを追加` | RELATES_TO | medium |

### Step 4: 詳細パネルで属性を設定

判断ノードを選択し、詳細パネルで：
- **決定タイプ**: `adopt`（採用の場合）
- **判断理由**: 「移植性行列の説明力不足から、分離構造が必須。方向エンコードは単独では効果未検証のため保留」
- **再評価トリガー**: 「方向エンコード単独の定量評価が出た場合」

### Step 5: 判断の読み方

- **Decision に `decision_type=adopt`** → 今期進めるPoC・実装の根拠
- **Decision に `decision_type=hold`** → 保留理由と再評価条件が明確
- **Decision に `decision_type=reject`** → なぜ見送ったかのログ（将来の参照用）
- **Decision に `decision_type=park`** → 中長期テーマへの移送記録

> **Idea と Decision の違い**: Idea は「できるかもしれない案」、Decision は「今回はこれで進める判断」。
> Idea が複数あるなかから1つの方針を選ぶ行為が Decision。両者を分けることで、研究ログの可読性が大幅に向上する。
...
- **Decision は軽く作る** — 判断理由と再評価条件の2行だけでも十分。重くすると使われなくなる
- **reject/park も残す** — 「なぜやらなかったか」は「なぜやったか」と同じくらい将来の自分に役立つ
